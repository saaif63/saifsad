...
email='coxab48957@dmsdmg.com'
pswd='amonguslivewallpape88'
c.login(email,pswd)
...
def on_message(data: samino.lib.Event):
	...
	if msg.startswith('!global'):
		if 'http' in msg:
			userrId=c.get_from_link(msg[8:]).objectId
			IIdd = client.get_user_info(userrId).aminoId
			local.send_message(chatId,f'http://aminoapps.com/u/{IIdd}',replyTo=msgId)
